...
(function () {
    angular.module('smartac.page')
        .controller('homeCtrl', ['$scope', '$sessionStorage', '$rootScope', '$ionicPopup', '$state',  'AJAX', '$ionicToast', '$q', '$getCode', '$getUrlParams', '$checkAuthorize', '$filter', '$log', '$ionicLoading', '$integralInfo', '$getMessage', '$localStorage', '$timeout', function ($scope, $sessionStorage, $rootScope, $ionicPopup, $state,  AJAX, $ionicToast, $q, $getCode, $getUrlParams, $checkAuthorize, $filter, $log, $ionicLoading, $integralInfo, $getMessage, $localStorage, $timeout) {
            ...
            if (!$rootScope.alreadyIn) {
                 ...
                 *
                 * directToState:'members','subNav.memberCoupon','subNav.memberIntegralQuery'
                 * needAuth:'true','false'
                 * stateParams:'String'
                 *
                 * 源码：
                 * http://vivouat.smartac.co/webapp/index.html?directToState=subNav.memberIntegralQuery&needAuth=true
                 * encodeURIComponent(“”)
                 * 转码：
                 * http%3A%2F%2Fvivouat.smartac.co%2Fwebapp%2Findex.html%3FdirectToState%3DsubNav.memberIntegralQuery%26needAuth%3Dtrue
                 *
                 * 店铺详情
                 * http://vivouat.smartac.co/webapp/index.html?directToState=subNav.brandDetail&needAuth=true&stateParams=44fbba54-2921-491a-a2d1-54ad5268ab7e
                 *
                 * 跳转到—>用户中心，并且验证Full权限
                 * https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx9587233d8b1a1869&redirect_uri=http%3A%2F%2Fvivouat.smartac.co%2Fwebapp%2Findex.html%3FdirectToState%3Dmembers%26needAuth%3Dtrue&response_type=code&scope=snsapi_base&state=index#wechat_redirect
                 *
                 *跳转到—>礼品卡券，并且验证Full权限
                 * https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx9587233d8b1a1869&redirect_uri=http%3A%2F%2Fvivouat.smartac.co%2Fwebapp%2Findex.html%3FdirectToState%3DsubNav.memberCoupon%26needAuth%3Dtrue&response_type=code&scope=snsapi_base&state=index#wechat_redirect
                 *
                 *跳转到—>积分查询，并且验证Full权限
                 * https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx9587233d8b1a1869&redirect_uri=http%3A%2F%2Fvivouat.smartac.co%2Fwebapp%2Findex.html%3FdirectToState%3DsubNav.memberIntegralQuery%26needAuth%3Dtrue&response_type=code&scope=snsapi_base&state=index#wechat_redirect
                 *
                 *
                 * */
                ...
                var stateParams = urlParams.stateParams;
                if (directToState) {
                    if (directToState.indexOf("subNav") != -1) {
                        if (!!needAuth) {
                            $checkAuthorize().then(function () {
                                stateGo(directToState,stateParams)
                            })
                        } else {
                            stateGo(directToState,stateParams)
                        }
                ...
                function stateGo(directToState,stateParams){
                    if(!!stateParams){
